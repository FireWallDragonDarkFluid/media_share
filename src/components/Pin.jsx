...
import {BsFillArroUpRightCircleFill, BsFillArrowUpRightCircleFill} from 'react-icons/bs'
...
const Pin = ({pin:{postedBy,image,_id,destination,save}}) => {
  ...
  const alreadySaved = !!save?.filter(item=>postedBy._id===user._id).length
        ...
        {postedHovered && (
          <div className='absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pb-2 z-50' style={{height:'100%'}}>
            <div className='flex items-center justify-between'>
              ...
              {alreadySaved ? (
                <button type="button" className='bg-red-500 opacity-70 hover:opacity-100 text-white font-bold px-5 py-1 text-base rounded-xl hover:shadow-md outline-none'>{save?.length}Saved</button>
              ):(
